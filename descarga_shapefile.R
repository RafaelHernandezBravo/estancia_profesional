...
dir_path = "archivos_shp_denue_11_2024"
...
for (i in 1:nrow(df)) {
  print(i)
  download_url <- df$descarga_shp[i] # Asumiendo que df$descarga_csv contiene las URLs
  ...
  tryCatch({
    download.file(download_url, destfile = paste0("archivos_shp_denue_11_2024/", file_clean, ".shp"), mode = "wb")
    print(paste("Descarga exitosa:", file_clean, ".shp"))
  }, error = function(e) {
    print(paste("Error al descargar", file_clean, ".shp:", e$message))
  })
}